#!/usr/bin/env python3
"""Faalt als de add-on-versie in hardlopen/config.yaml niet omhoog is gegaan
t.o.v. een eerdere git-ref.

Voorkomt dat een (per ongeluk lagere of gelijke) versie naar master gepusht
...
CONFIG_PATH = "hardlopen/config.yaml"
...
def main():
    ...
    if new_version <= old_version:
        raise SystemExit(
            ...
            "update.hardlopen_update-entity in Home Assistant -- die kan hoger "
            "zijn dan wat de git-geschiedenis laat zien -- en verhoog 'version' "
            "in hardlopen/config.yaml voldoende."
        )
